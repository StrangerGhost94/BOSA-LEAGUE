...
export default async function LiveDeskHome() {
  ...
  const Section = ({ title, list, empty }: { title: string; list: M[]; empty?: string }) => (
    <section className="mb-7">
      ...
      {list.length ? <div className="space-y-3">{list.map((m) => <Card key={m.id} m={m} />)}</div> : empty && <p className="text-sm text-ivory/40">{empty}</p>}
    </section>
  );
  return (
    <>
